**useOpenRouter API Documentation v0.1.0**

***

# useOpenRouter API Documentation v0.1.0

## Interfaces

- [ChatMessage](interfaces/ChatMessage.md)
- [ChatOptions](interfaces/ChatOptions.md)
- [ChatResponse](interfaces/ChatResponse.md)
- [ChatStats](interfaces/ChatStats.md)
- [OpenRouterError](interfaces/OpenRouterError.md)
- [OpenRouterModel](interfaces/OpenRouterModel.md)
- [OpenRouterOptions](interfaces/OpenRouterOptions.md)
- [UseOpenRouterReturn](interfaces/UseOpenRouterReturn.md)

## Type Aliases

- [ModelID](type-aliases/ModelID.md)
- [Temperature](type-aliases/Temperature.md)

## Functions

- [useOpenRouter](functions/useOpenRouter.md)
